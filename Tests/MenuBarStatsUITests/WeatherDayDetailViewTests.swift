...
struct WeatherDayDetailViewTests {
    ...
    func hostedLayout() throws {
        ...
        for (name, appearance) in [("light", NSAppearance.Name.aqua), ("dark", .darkAqua)] {
            let view = NSHostingView(rootView: WeatherDayDetailView(
            ...
            if let directory = ProcessInfo.processInfo.environment["WEATHER_SNAPSHOT_DIRECTORY"] {
                ...
                let renderer = ImageRenderer(content: content.forecastContent
                    .frame(width: 380)
                ...
                let chartRenderer = ImageRenderer(content: HourlyForecastChart(
                ...
                try chartData.write(to: URL(fileURLWithPath: directory)
                    .appendingPathComponent("weather-chart-\(name).png"))
                if let point = forecast.hourly.first {
                    let hourlyRenderer = ImageRenderer(content: content.hourlyMetrics(point)
                    ...
                    try hourlyData.write(to: URL(fileURLWithPath: directory)
